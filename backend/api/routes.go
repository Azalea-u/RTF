...
func NewRouter(db *db.Database) *http.ServeMux {
    ...
    // Handling GET and POST separately for messages
    r.Handle("/api/messages", wrap(m.AuthMiddleware(http.HandlerFunc(h.GetMessages)))) // Get messages - GET
    r.Handle("/api/messages/send", wrap(m.AuthMiddleware(http.HandlerFunc(h.SendMessage)))) // Send message - POST
